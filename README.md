...
graph TD
    ...
    main --> MX_GPIO_Init
    main --> MX_USART1_UART_Init
    main --> MX_I2C1_Init
    main --> MX_TIM2_Init
    main --> DHT22_initialize
    main --> OLED_Init
    main --> Relay_initialize
    main --> TempControl_Init
    main --> xSemaphoreCreateMutex
    main --> xTaskCreate_SensorTask
    main --> xTaskCreate_DisplayTask
    main --> xTaskCreate_ControlTask
    main --> vTaskStartScheduler

    xTaskCreate_SensorTask --> SensorTask
    SensorTask --> DHT22_readData
    SensorTask --> protectedSensorDataAccess
    protectedSensorDataAccess --> SensorData_updateData
    SensorTask --> printMessage

    xTaskCreate_DisplayTask --> DisplayTask
    DisplayTask --> OLED_Clear
    DisplayTask --> protectedSensorDataAccess
    DisplayTask --> OLED_ShowTemperature
    DisplayTask --> OLED_ShowHumidity
    DisplayTask --> OLED_ShowString
    DisplayTask --> OLED_ShowSystemStatus
    DisplayTask --> OLED_Refresh

    xTaskCreate_ControlTask --> ControlTask
    ControlTask --> protectedSensorDataAccess
    ControlTask --> Controller_update
    Controller_update --> executeAutoMode
    executeAutoMode --> executeTemperatureControl
    executeTemperatureControl --> Relay_setState
    executeAutoMode --> executeHumidityControl
    executeHumidityControl --> Relay_setState
    Controller_update --> executeManualMode
    Controller_update --> Relay_safetyCheck

    DHT22_initialize --> Sensor_setPinOutput
    DHT22_readData --> Sensor_setPinOutput
    DHT22_readData --> Sensor_setPinInput
    DHT22_readData --> Sensor_setPin
    DHT22_readData --> Sensor_readPin
    DHT22_readData --> Sensor_delayUs
    DHT22_readData --> Sensor_getMicros

    Relay_initialize --> Relay_turnOffAll
    Relay_setState --> Relay_setPhysicalState
    Relay_safetyCheck --> Relay_setState
    Relay_turnOffAll --> Relay_setPhysicalState
    Relay_emergencyStop --> Relay_turnOffAll
```
...
- **周期**：每2秒执行一次
- **主要流程**：
  1. 调用 DHT22_readData 读取温湿度数据。
  2. 检查数据有效性（温度/湿度范围）。
  3. 通过 protectedSensorDataAccess 互斥访问，调用 SensorData::updateData 更新全局数据。
  4. 读取失败时，调用 SensorData::invalidate 标记数据无效。
  5. 通过串口输出调试信息。
- **线程安全**：所有对全局传感器数据的写操作都通过互斥量保护，避免多任务竞争。

#### 2. DisplayTask（OLED显示任务）
- **周期**：每0.5秒执行一次
- **主要流程**：
  1. 通过 protectedSensorDataAccess 互斥读取最新的 SensorData。
  2. 调用 OLED_Clear 清空显示缓冲区。
  3. 如果数据有效，调用 OLED_ShowTemperature、OLED_ShowHumidity 显示温湿度，调用 OLED_ShowSystemStatus 显示设备状态。
  4. 如果数据无效，调用 OLED_ShowString 显示错误提示。
  5. 最后调用 OLED_Refresh 刷新屏幕。
- **线程安全**：所有对全局传感器数据的读操作都通过互斥量保护，确保显示内容一致。

#### 3. ControlTask（智能控制任务）
- **周期**：每1秒执行一次
- **主要流程**：
  1. 通过 protectedSensorDataAccess 互斥读取最新的 SensorData。
  2. 检查数据有效性和新鲜度（如5秒内更新）。
  3. 调用 TemperatureController::update 执行智能控制决策：
     - 自动模式下，分别调用 executeTemperatureControl 和 executeHumidityControl，利用 PID 算法计算输出，控制继电器（加热、风扇、加湿）。
     - 手动/关闭模式下，执行对应策略。
  4. 调用 Relay::Controller::safetyCheck 实现安全保护（如加热器超时自动断电）。
  5. 通过串口输出控制和安全状态。
- **线程安全**：所有对全局传感器数据的读操作都通过互斥量保护，控制决策基于最新可靠数据。
