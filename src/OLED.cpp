...
static OLED_Status_t writeData_C(const uint8_t* buffer, uint16_t len) {
  ...
  HAL_StatusTypeDef status =
      HAL_I2C_Master_Transmit(&hi2c1, 0x78, tempBuffer, len + 1, 1000);
...
static void formatFloat_C(char* buffer, size_t bufferSize, float number,
                          uint8_t precision) {
...
class OLEDController {
 public:
  static OLED_Status_t initialize() {
    HAL_Delay(100);
    const uint8_t initCommands[] = {OLED_CMD_DISPLAY_OFF,
                                    OLED_CMD_SET_CLOCK_DIV,
                                    0x80,
  ...
  static OLED_Status_t showString(uint8_t x, uint8_t y, const char* str,
                                  uint8_t fontSize) {
  ...
  static OLED_Status_t showNumber(uint8_t x, uint8_t y, float number,
...
OLED_Status_t OLED_ShowString(uint8_t x, uint8_t y, const char* str,
                              uint8_t fontSize) {
  return ReptileController::Display::OLEDController::showString(x, y, str,
                                                                fontSize);
}

OLED_Status_t OLED_ShowNumber(uint8_t x, uint8_t y, float number,
                              uint8_t precision, uint8_t fontSize) {
  return ReptileController::Display::OLEDController::showNumber(
      x, y, number, precision, fontSize);
}

OLED_Status_t OLED_ShowTemperature(float temperature, float target) {
  return ReptileController::Display::OLEDController::showTemperature(
      temperature, target);
}

OLED_Status_t OLED_ShowHumidity(float humidity, float target) {
  return ReptileController::Display::OLEDController::showHumidity(humidity,
                                                                  target);
}
